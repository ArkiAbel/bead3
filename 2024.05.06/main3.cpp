...
//osszdata
struct Reserved_left {
    int x, y;
    string id;
};

struct Reserved_right {
    int x, y;
    string id;
};
...
class Ships : public Sea {
    ...
    void generate_ships_left(vector<Reserved> &r);
    void generate_ships_right(vector<Reserved> &r);
};
...
void Ships::generate_ships_left(vector<Reserved> &r) {
    Reserved res;
...
int main() {
    ...
    vector<Reserved_left>rl;
    vector<Carrier>cv;
    vector<Battleship>bv;
    vector<Cruiser>crv;
    vector<Submarine>sv;
    vector<Destroyer>dv;
    vector<Reserved_left>rr;

    vector<string> b;
    vector<int> sz;

    Ships sh = Ships(0,0,false,cv,bv,crv,sv,dv);
    sh.generate_ships_left(rl,rr);
    sh.generate_ships_right(rl,rr);
    Sea s = Sea(0,0,false);
    s.win_open(XX,YY);
    s.info_load(b,sz);
    s.event_loop(b,sz,rl,rr);
